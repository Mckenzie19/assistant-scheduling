...
public class DateSelectionPanel extends FormPanel {
	...
	private LocalDate[] serviceDateRange;
	private LocalDate[] additionalServices;
	private DataFileCreator dfc;
	{
	...
	public DateSelectionPanel(DataFileCreator dfc) {
		...
		txtAdditionalServices.setBorder(new LineBorder(Color.GRAY));
		tpAddServices.setForeground(Color.LIGHT_GRAY);
		warningLabel.setFont(StyleSettings.WARNING_FONT);
		...
		add(warningLabel, "span 3,shrinkx,wrap");
	}
	...
	public void checkData() {
		...
		if (allDataEntered()) {
			...
			if (dateRangeLogical(startDate, endDate)) {
				...
				// TODO: Add warnings here for user to see
			}
		} else {
			// TODO: Add warnings here for user to see
		}
